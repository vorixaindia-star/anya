  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 pt-20">
      <div className="flex items-center justify-between mb-8">
        ...
        <CosmicButton glow onClick={() => router.push('/instructor/live/schedule')}>
          + Schedule New Class
        </CosmicButton>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {classes.map((cls: any) => {
                const isLive = new Date(cls.scheduled_at) <= new Date() && 
                               new Date(new Date(cls.scheduled_at).getTime() + cls.duration * 60000) > new Date()
                const status = isLive ? 'live' : 
                               new Date(cls.scheduled_at) > new Date() ? 'upcoming' : 'completed'
                return (
                  <LiveClassCard
                    key={cls.id}
                    id={cls.id}
